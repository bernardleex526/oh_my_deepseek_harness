...
/**
 * Render the preset-local runtime catalog.
 *
 * This file is imported by `orchestration.mjs`; it must list exactly the
 * delegation tools compiled into this composition so the Orchestrator
 * allow-list (and the writer-lock set) stays in sync with custom roles.
 * @param {Array<object>} specialists - the specialists in this build.
 * @returns {string} the runtime catalog module text.
 */
export function renderRuntimeCatalog(specialists = SPECIALISTS) {
	const tools = specialists.map((s) => s.toolName);
	const writers = specialists
		.filter((s) => {
			const allow = s.filterFor("posix").allow;
			return allow.includes("write") || allow.includes("edit");
		})
		.map((s) => s.toolName);
	return [
		"/** GENERATED BY scripts/build.mjs — edit src/ and prompts/, then rebuild. */",
		"export const DELEGATION_TOOLS = [",
		...tools.map((name) => `\t${JSON.stringify(name)},`),
		"];",
		"",
		"export const WRITER_DELEGATION_TOOLS = [",
		...writers.map((name) => `\t${JSON.stringify(name)},`),
		"];",
		""
	].join("\n");
}
...
export function build() {
	...
	const runtimeCatalogPath = join(PRESET_DIR, "runtime-catalog.mjs");
	...
	const bootstrapPath = join(PRESET_DIR, "bootstrap.mjs");
	// Dist builds never read model-routing.json/roles.json; local builds do.
	const customRoles = mode === "local" ? loadCustomRoles(ROOT) : [];
	const specialists = mode === "local" ? [...SPECIALISTS, ...customRoles] : SPECIALISTS;
	...
	if (mode === "local" && customRoles.length > 0) {
		writeFileSync(runtimeCatalogPath, renderRuntimeCatalog(specialists), "utf8");
	} else {
		copyFileSync(join(ROOT, "src", "orchestration", "runtime-catalog.mjs"), runtimeCatalogPath);
	}
	...
	copyFileSync(join(ROOT, "src", "orchestration", "bootstrap.mjs"), bootstrapPath);
	return { written: [compositionPath, metadataPath, rowPath, runtimeCatalogPath, brokerPath, protocolPath, artifactsPath, policyPath, bootstrapPath], mode };
}
